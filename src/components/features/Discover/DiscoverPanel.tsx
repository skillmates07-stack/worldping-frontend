...
import { Compass, X, TrendingUp, MapPin, Zap, Trophy, Filter, Minimize2, Maximize2 } from 'lucide-react'
import { supabase } from '@/lib/supabase/client'
import { useDeviceId } from '@/hooks/useDeviceId'
import { formatTimeAgo } from '@/lib/utils'
import MessageCard from '@/components/features/MessageFeed/MessageCard'
...
type DiscoverTab = 'nearby' | 'trending' | 'events' | 'leaderboard'
...
export default function DiscoverPanel() {
  ...
  async function fetchNearbyMessages() {
    ...
    // Fetch messages and calculate distance
  ...
  async function fetchLeaderboard() {
    // Get top users by total upvotes
    const { data, error } = await supabase
      .from('messages')
    ...
    // Group by device_id and calculate score
                      ...
                      <>
                        {messages.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-full text-center px-4">
                            ...
                            <p className="text-gray-600 text-xs mt-1">Drop a message to see it here!</p>
                          </div>
                        ) : (
                          messages.map(msg => (
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                                    <span>📍 {msg.distance?.toFixed(1)}km away</span>
                      ...
                      <>
                        {messages.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-full text-center px-4">
                            ...
                            <p className="text-gray-400 text-sm font-medium">No trending messages</p>
                          </div>
                        ) : (
                          messages.map((msg, i) => (
                            <div key={msg.id} className="bg-gray-800 rounded-xl p-3 border border-gray-700">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  <p className="text-white text-sm">{msg.content}</p>
                                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                                    <span>🔥 {msg.upvotes - msg.downvotes} points</span>
                            ...
                            <div key={event.id} className="bg-gray-800 rounded-xl p-3 border border-red-500/50">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-gray-600 text-xs mt-2">{formatTimeAgo(event.created_at)}</p>
                                </div>
                              </div>
                            </div>
                          ))
                    ...
                    {activeTab === 'leaderboard' && (
                      <>
                        {leaderboard.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-full text-center px-4">
                            <Trophy className="w-12 h-12 text-gray-700 mb-2" />
                            <p className="text-gray-400 text-sm font-medium">No rankings yet</p>
                          </div>
                        ) : (
